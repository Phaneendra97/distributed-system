...
def result():
    ...
    for task_nbr in range(10000):
        ...
        print("Worker %i" %result_message['worker'])
        print("answered: %i" %result_message['result'])
...
if __name__ == "__main__":
    ...
    for wrk_num in range(len(worker_pool)):
        worker_manager = Process(target=worker, args=(wrk_num))
        worker_manager.start()

        # Fire up our result manager...
        result_manager = Process(target=result, args=())
        result_manager.start()
    
        # Start the ventilator!
        ventilator_manager = Process(target=ventilator, args=())
        ventilator_manager.start()             